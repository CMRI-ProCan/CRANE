...
# ProteomeXchange (Identifier—PXD020529)
# 
# ======================================================================
import os
import sys
...
file_list = file_list.loc[file_list.File_List.str.contains('64fix')]
file_list = file_list.File_List.to_list()

irt_srl_path = 'srl/Navarro_openSwathIrt_64w_fix.tsv'
srl_path = 'srl/Navarro_openSwathDecoySrlTsv_64w_fix.tsv'
    ...
    if len(tof_path) > 0:
        denoiser = App(
            ...
            denoised_fname=tof_path.replace(
                '.tof',
                '.crane_MS1_{}_MS2_{}_64fix.tof'.format(param_set1, param_set2)
            ),
            window_name=None,
            include_ms1=True,
        )
