...
file_list = pd.read_csv('Matrisome_raw_file_details.csv')
file_list = file_list['wiff_file_name'].to_list()

irt_srl_path = 'srl/Matrisome_openSwathIrt.tsv'
srl_path = 'srl/Matrisome_openSwathDecoySrlTsv.tsv'
    ...
    if len(tof_path) > 0:
        denoiser = App(
            ...
            denoised_fname=tof_path.replace(
                '.tof',
                '.crane_V3_MS1_{}_MS2_{}.tof'.format(param_set1, param_set2)
            ),
            window_name=None,
            include_ms1=True,
        )
...
for param_set1 in MS1_settings_list:
    ...
    for file in file_list:
        file_name = file.split('.')[0] + '.tof'
        message = create_CRANE_denoised_file(file_name, param_set1, param_set2)
        print(message)